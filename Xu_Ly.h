...
//luu node thuoc cay vao file 
void Save(TREE_DG t, fstream &file);

// duyet cay NLR de luu het mo node trong cay 
void OutFile_DG(TREE_DG t, fstream &file);

// dem nguyen cai cay luu vao file 
...
// ouput tree vao file theo kieu duyet NLR
void OutFile_DG(TREE_DG t, fstream &file){
	if (t != NULL){
		Save(t, file);
...
void Save_DS(LIST_DAUSACH lDS){
	...
	if (outFile.is_open()){
		...
		for (int i = 0; i <= lDS.n; i++){
			...
			outFile << lDS.ListDS[i]->info.luotmuon << endl;
...
label:
	...
	do
	{
		...
		choose1 = ChooseItems_DS(lDS, tttrang, tongtrang);
        ...
        check = SearchMaSach_MT(nDG->mt, masach);
		// check cac truong hop
		if (check == true)
		{
		...
		label1:
			...
			for (NODE_MT *p = temp->mt.pHead; p != NULL ; p = p->pNext)
			{
				...
				if (p->data.trangthai == 0  || p->data.trangthai == 2)
				{
					...
					xuatNGAY_THANG(p->data.ngaymuon, 118, 20 + i);
					i++;
					if (i == 3)
					{
			...
			if (MatSach(temp->mt))
			{
				...
				temp->DG.trangthaithe = 0;
				_getch();
				return;
			}
			// xu ly truong hop muon sach qua han 7 ngay.
			if (SoNgayQuaHan(temp->mt)>0)
			{
				chuChay(thongbao3, 90, 24);
				temp->DG.trangthaithe = 0;
				_getch();
			...
			else if (check2 == 1)
			{
				...
				do
				{
					...
					check3 = NhapNGAY_THANG(mt.ngaymuon, 100, 30);
		...
		label1:
			...
			do
			{
				while (_kbhit())
				{
					...
					{
					case  KEY_F3:
						choose1 = chonItem_MT(temp->mt,lDS);
						if(choose1==-2) return;
						if (choose1 == -1)
						{
							...
							if (pMT->data.trangthai == 0 || pMT->data.trangthai == 2)
							{
								...
								if (i1 == choose1)
								{
									...
									{										
										...
										check1 = NhapNGAY_THANG(ngaytra, 55, 30);
										...
										// check cac truong hop
										 if (check1 == 2)
										 {
											 ...
											 else
											 {
												 ...
												 //pDAU_SACH pDS = SearchDSByTen(lDS, pMT->data.tensach);
                                                 pDAU_SACH pDS = SearchMASACH_DS(lDS,pMT->data.MASACH);
					
												 // tim kiem sach dua tren ma dau sach ma doc gia da muon.
												 NODE_DMS* pDMS = Search_DMS1(pDS, pMT->data.MASACH);
                                                 
												 // thay doi trang thai sach sang CHO MUON DUOC
												 pDMS->data.ttSach = 0;
												 // mo khoa the neu ko vi pham gi
												 if(MatSach(temp->mt) == 0 && SoNgayQuaHan(temp->mt)<=0) temp->DG.trangthaithe = 1;
												 goto label1;
											 }
										 }
										 else if (check1 == -1)
					...
					case KEY_F4: 
						//Beep(600, 100);
						choose2 = chonItem_MT(temp->mt,lDS);
						if(choose2==-2) return;
						if (choose2 == -1)
						{
							chuChay(thongbao, 50, 30);
...
void MainMenu(TREE_DG &t, LIST_DAUSACH &lDS)
{
	...
	// set ten cua so
	SetConsoleTitle(_T("QUAN LY THU VIEN 1 "));
