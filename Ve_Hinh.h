...
string keyGuide2[7] = { "                     ~~~HuongDan~~~                   ",
						"-> TEN SACH     : Toi da 24 ky tu, gom co chu va so ",
						"-> ISBN         : Nhap 4 ky tu, chi lay ky tu chu    ",
...
int xMainMenus[5] = { 9,40,66, 95,119 };
int ySubMainMenus1[3] = { 15, 19, 23};
int ySubMainMenus2[3] = { 15, 19, 23 };
int ySubMainMenus3[3] = { 15,18,21 };
...
void VeKhungMenu()
{
	...
	// ve hang doc trai
	for (int i = 0; i < yOrdinal + 5; i++)
	{
		gotoxy(xOrdinal + 30, yOrdinal + i);
		SetBGColor(BRIGHT_WHITE );
		cout << "  ";
	}
...
void SubMainMenu1(string key[], int nKey)
{
	...
	HighLight1(keySubMainMenu1[0], 0, xSubMainMenus1, ySubMainMenus1, LIGHT_PURPLE , BLACK);
	ShowCur(false);
}
...
void SubMainMenu2(string key[], int nKey)
{
	...
	HighLight1(keySubMainMenu2[0], 0, xSubMainMenus2, ySubMainMenus2, LIGHT_PURPLE , BLACK);
	ShowCur(false);
}
...
void SubMainMenu3(string key[], int nKey)
{
	...
	HighLight1(keySubMainMenu3[0], 0, xSubMainMenus3, ySubMainMenus3, LIGHT_PURPLE , BLACK);
	ShowCur(false);
}
...
void EffectiveMenu(int pos, int flag)
{
	// 0 --> 3; 3 --> 2;   2---> 1;  1---> 0
	if (flag == 1)
	{// di chuyen thanh sang bang right
	 // 0 --> 3; 3 --> 2;   2---> 1;  1---> 0
		ShowCur(false);
		gotoxy(xMainMenus[pos + 1] - 1, yMainMenu);
		SetBGColor(LIGHT_PURPLE );
		SetColor(WHITE);
		...
		//di chuyen thanh sang bang left
		// 0 ---> 1; 1 ---> 2; 2 ----> 3 ; 3 ---> 0
		ShowCur(false);
