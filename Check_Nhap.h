...
int NhapNGAY_THANG(NGAY_THANG &nt, int x, int y);

// ham ho tro nhap ma dau sach trong danh muc sach
int NhapMaSach(string &result);
	...
	while (true)
	{
		...
		while (_kbhit())
		{
			...
			else if (kb_hit == ESC)
			{
				...
				gotoxy(X_NOTIFY + 15, Y_NOTIFY);
				SetColor(BLUE);
				cout << "NHAP 4 KY TU, CHI LAY KY TU CHU !!!";
				normalBGColor();
				gotoxy(x, y);
			}
...
int  NhapMaSach(string &result)
{
	...
	while (true)
	{
		...
		while (_kbhit())
		{
			...
			// chi nhap ten toi da 10 ky tu, vi tu dai nhat tieng viet chi la 10.
			if (((kb_hit >= 65 && kb_hit <= 90) || (kb_hit >= 97 && kb_hit <= 122) || (kb_hit >= 48 && kb_hit <= 57)
